...
public class TrieApp {
    ...
    public void printTopKDescending(TrieNode root , int k) {
    	MinHeap heap = new MinHeap(k);
    	
    	collectWords(root , "" ,heap);
    	
    	if(heap.size == 0) {
    		System.out.println("No words inside the heap");
    	}
    	
    	printRecursively(heap , k);	
    }
    
    private void printRecursively(MinHeap heap , int k) {
    	if(k == 0 || heap.size == 0)
    		return;
    	
    	String minWord = heap.removeMin();
    	
    	printRecursively(heap , k-1);
    	
    	System.out.println(minWord);
    }
    
    private void collectWords(TrieNode node , String currentWord , MinHeap heap) {
    	if(node == null)
    		return;
    	...
    	for(RobinHoodHashing.Element element : node.children.table)
    		if(element != null)
    			collectWords(element.trieNode , currentWord + element.key , heap);
    }
    */

    /* $$SOLUTION OF EXERCISE 2$$
    public void topKWordsWithPrefix(TrieNode root ,String prefix, int k) {
    	TrieNode prefixNode = findPrefix(root , prefix , 0);
    	
    	if(prefixNode == null) {
    		System.out.println("No such a prefix inside TrieNode");
    		return;
    	}
    	
    	MinHeap heap = new MinHeap(k);
    	
    	collectWords(prefixNode , prefix , heap);
    	
    	printRecursively(heap , k);
    		
    }
    
    private TrieNode findPrefix(TrieNode node , String prefix , int index) {
    	if(node == null)
    		return null;
    	...
    	for(RobinHoodHashing.Element element : node.children.table) {
    		if(element != null && element.key == prefix.charAt(index)) {
    			newNode = element.trieNode;
    			break;
    		}
    	}
    	...
    	return findPrefix(newNode , prefix , index + 1);
    }
    ...
    */
    ...
    /*$$SOLUTION OF EXERCISE 3$$
  	public int countWords(TrieNode node){
  		...
  		for(RobinHoodHashing.Element element : node.children.table)
  			if(element != null)
  				totalWords += countWords(element.trieNode);
    ...
    */
    ...
    public float averageImportance(TrieNode root) {
    	return countImportance(root)/(float)countWords(root);
    }
    
    private int countImportance(TrieNode node) {
    	if(node == null)
    		return 0;
    	...
    	for(RobinHoodHashing.Element element: node.children.table)
    		if(element != null)
    			totalImportance += countImportance(element.trieNode);
    ...
    */
    ...
    /*$$$BONUS EXERCISE$$$
    public int longestWord(TrieNode node) {
    	if(node == null)
    		return 0;
    	...
    	if(node.wordLength > 0)
    		longest = node.wordLength;
    	
    	for(RobinHoodHashing.Element element : node.children.table) {
    		if(element != null) {
    			int childsLongest = longestWord(element.trieNode);
    			
    		if(childsLongest > longest)
    			longest = childsLongest;
    		}
    	}
    	return longest;
    }
    */
    ...
    public String actualLongestWord(TrieNode node , String currentWord) {
    	if(node == null)
    		return null;
    	...
    	for(RobinHoodHashing.Element element : node.children.table) {
    		if(element != null) {
    			String childLongestWord = actualLongestWord(element.trieNode , currentWord + element.key);
    ...
    */
    ...
    public static void main(String[] args) {
        ...
        trie.printTopKDescending(trie.root, k);*/
        ...
        trie.topKWordsWithPrefix(trie.root, prefix, k);
        */
        
        /*$$FOR EXERCISE 3$$
        int totalWords = trie.countWords(trie.root);
        System.out.println("Total words in Trie: " + totalWords);
        */
        
        /*$$FOR EXERCISE 4$$
        float averageImportance= trie.averageImportance(trie.root);
        System.out.println("Total importance in Trie: " + averageImportance);
        */
